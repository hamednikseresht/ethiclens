...
export function verificationEmail({ name, url }) {
  return {
    subject: `${getSetting('site_title') || 'Ethic Lens'} — تأیید نشانی ایمیل`,
    html: layout({
      ...
      buttonUrl: url,
      rawUrl: url,
      footer: 'اگر شما در دیدگاه اخلاق ثبت‌نام نکرده‌اید، این ایمیل را نادیده بگیرید؛ بدون تأیید، حسابی فعال نمی‌شود.'
    })
  };
}
