import { md } from './render-analysis.js';
import { escapeHtml as esc } from './seo.js';

/**
 * Server-side rendering of the encyclopedia.
 *
 * This is the site's largest piece of original writing — eight lenses, five
 * gates, six thought experiments, all cited — and until now it existed only
 * as a fetch from /api/guide, so the first response was a skeleton and a
 * crawler saw an empty div where 32KB of Persian philosophy should be. It is
 * also the page most likely to be linked to, which makes it the worst page on
 * the site to hide behind JavaScript.
 *
 * A port of public/js/guide.js, structure for structure, so the markup and
 * the stylesheet stay in step. The client copy is gone: rendering it twice
 * would only replace identical HTML after a round trip the reader waited for.
 *
 * Only the text comes from the database. The structure is fixed here, so an
 * admin editing a section cannot break the page.
 */
