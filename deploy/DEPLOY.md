...
اسکریپت همه گام‌ها را انجام می‌دهد و روی اولین خطا متوقف می‌شود.

> **چرا اسکریپت، نه چهار دستور دستی:** اجرای گام‌ها در خطوط جدا دو جور
> خراب می‌شود که هر دو شبیه موفقیت‌اند.
>
> اول، اگر نشانی مخزن نام کاربری داخل خودش داشته باشد
> (`https://user@github.com/...`)، گیت مجبور به احراز هویت می‌شود و
> گیت‌هاب از اوت ۲۰۲۱ رمز حساب را نمی‌پذیرد — همیشه `HTTP 401`. اسکریپت
> نشانی را خودش اصلاح می‌کند، پس این اشتباه خودبه‌خود درمان می‌شود.
> (نشانی‌هایی که توکن دارند دست‌نخورده می‌مانند، چون ممکن است احراز هویت
> واقعی یک مخزن خصوصی باشند.)
>
> دوم، `git pull` که شکست بخورد جلوی `npm ci` و ری‌استارت را نمی‌گیرد.
> خروجی پر از پیام موفقیت می‌شود در حالی که کد اصلاً عوض نشده — دقیقاً
> همان حالتی که «۷۹ پکیج نصب شد» را می‌بینید و فکر می‌کنید به‌روز شده‌اید.
>
> اسکریپت در پایان هم بررسی می‌کند سرویس واقعاً بالا مانده و به
> `/api/health` پاسخ می‌دهد — ری‌استارتی که موفق به نظر برسد و بعد روی یک
> مهاجرت خراب بیفتد، همان چیزی است که باید گرفته شود.

اگر ترجیح می‌دهید دستی بزنید، حتماً با `&&` زنجیر کنید:
